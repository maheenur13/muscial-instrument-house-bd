    ...
    const onSubmit = data => {
        ...
        if (oldUser && user.email && user.password) {
            firebase.auth().signInWithEmailAndPassword(user.email, user.password)
                .then(res => {
                    // Signed in
                    console.log('current user all info',res.user)
                    const newUserInfo = { ...user };
                    newUserInfo.error = '';
                    newUserInfo.name =res.user.displayName;
                    newUserInfo.success = true;
                    setUser(newUserInfo);
                    console.log('logged is usere info',newUserInfo);
                    setLoggedInUser(newUserInfo);
                    history.replace(from);
                    history.push('/home');
                })
                ...
                <div className="col-md-7">
                    <div className="login-page-design ">
                        <form className="form-design" onSubmit={handleSubmit(onSubmit)}>
                            ...
                            { !oldUser && <input onBlur={handleFormValid} className="form-item" name="name" placeholder="Enter Your Name" {...register("name", { required: true, maxLength: 20 })} />}
                            {!oldUser && errors.name && <span className="error-design">Name is required</span>}
                            <input onBlur={handleFormValid} name="email" type="email" className="form-item" placeholder="Enter Your Email" {...register("email", { required: true })} />
                            {errors.email && <span className="error-design">Email is required </span>}

                            <input onBlur={handleFormValid} name="password" placeholder="password" className="form-item" type="password" {...register("password", { required: true })} />
                            {errors.password && <span className="error-design">Password is required</span>}

                            {!oldUser && <input onBlur={handleFormValid} name="confirmPassword" className="form-item" placeholder="Confirm password" type="password" {...register("confirmpassword")} />}
